...
const freeFontUrl = "https://free-font.zsnmwy.net/js/data.js";
https
  .get(freeFontUrl, (response) => {
    let str = "";
...
function generateCSS(list, data) {
  let freeFont = "";
  list.forEach((key) => {
    const newDownload = data[key].download.replace(
      "https://github.com/wordshub/free-font/raw/master/",
      "https://free-font.zsnmwy.net/"
    );
    freeFont += `@font-face {
...
function generateLocalCSS(list, data) {
  let freeFont = "";
  list.forEach((key) => {
    const src = data[key].download.replace(
      "https://github.com/",
      "http://localhost:3000/fontFile/"
    );
    freeFont += `@font-face {
          font-family: '${data[key].name}';
          src: url('${src}');
...
function generateJSON(list, data) {
  ...
  list.forEach((key) => {
    ...
    newData[key].preview =
      "https://free-font.zsnmwy.net/" + newData[key].preview;
  });
